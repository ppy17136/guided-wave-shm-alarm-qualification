"""Execute one submission-safe P1 shape-normalization job.
...
class ZarrWaveDataset(Dataset):
    def __init__(
        self, store: Path, indices: np.ndarray, mean: np.ndarray, std: np.ndarray,
        normalization_mode: str = "train_global",
    ):
        self.store = store
    ...
    def __getitem__(self, item: int):
        ...
        if self.normalization_mode == "train_global":
            wave = (wave - self.mean[:, None]) / (self.std[:, None] + 1e-6)
        elif self.normalization_mode == "instance_path_rms":
            scale = np.sqrt(np.mean(np.square(wave), axis=1, keepdims=True))
            wave = wave / (scale + 1e-6)
        elif self.normalization_mode == "instance_path_centered_rms":
            wave = wave - wave.mean(axis=1, keepdims=True)
            scale = np.sqrt(np.mean(np.square(wave), axis=1, keepdims=True))
            wave = wave / (scale + 1e-6)
        elif self.normalization_mode == "instance_path_centered_rms_derivative":
            wave = wave - wave.mean(axis=1, keepdims=True)
            scale = np.sqrt(np.mean(np.square(wave), axis=1, keepdims=True))
            wave = wave / (scale + 1e-6)
            wave = np.diff(wave, axis=1, prepend=wave[:, :1])
            derivative_scale = np.sqrt(np.mean(np.square(wave), axis=1, keepdims=True))
            wave = wave / (derivative_scale + 1e-6)
        else:
            raise ValueError(f"Unsupported wave normalization mode: {self.normalization_mode}")
        return {
...
def train_seed(config: dict[str, object], seed: int, smoke: bool, force: bool) -> dict[str, object]:
    ...
    try:
        ...
        normalization_mode = str(config.get("method", {}).get("wave_normalization", "train_global"))
        train_loader = make_loader(ZarrWaveDataset(train_store, split["train"], mean, std, normalization_mode), batch_size, workers, True)
        cal_loader = make_loader(ZarrWaveDataset(train_store, split["calibration"], mean, std, normalization_mode), batch_size, workers, False)
        test_loader = make_loader(ZarrWaveDataset(test_store, split["test"], mean, std, normalization_mode), batch_size, workers, False)
        ...
        torch.save({
            "model_state": best_state, "config": config, "wave_mean": mean, "wave_std": std,
            "wave_normalization": normalization_mode,
        }, out / "checkpoint.pt")
...
def main() -> None:
    ...
    if config["readiness"]["runner"] not in {
        "masked_reconstruction_anomaly", "masked_reconstruction_shape_anomaly"
    }:
